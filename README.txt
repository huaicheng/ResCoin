...
Source Tree:
    ...
    * ringbuffer.c: simple ring buffer implementation used to store historic workload data
...
|		〖ac〗_i (t) +=〖tc〗_i (t-1) - 〖au〗_i (t-1) - [〖tc〗_i (t-k-1)-〖au〗_i (t-k-1)]; 
|		for each VMi in vmlist; do // do the same thing for each resource
|			if (∑_(i=0)^n 〖est〗_i   ≤ C_h  && 〖est〗_i≤ 〖tc〗_i); then // tc satisfied
|				allocate(〖est〗_i);
|			else
|				C_e= C_h-∑_(i=0)^(n-1)〖min(〖est〗_i (t-1),〖tc〗_i (t))〗
|				〖wr〗_i=〖∥ac〗_i∥∕∑_(i=0)^n ∥〖ac〗_i 〗∥; // resource weight calibration
